...
### OQ-004 — SCORM 2004 + cmi5 real-LMS conformance — cmi5 RESOLVED (@xapi/cmi5); SCORM 2004 ACCEPTED KNOWN-LIMITATION (2026-06-18) — REOPENED OQ-001
...
- **Blocking?:** no. cmi5 resolved; SCORM 2004 completion rollup is a **deliberate, accepted known-limitation** — the toolkit ships on SCORM 1.2 + cmi5 + web (decision: Jim, 2026-06-18). The net (Task 10) is complete and is what surfaced this.
- **Resolution (2026-06-18):**
  ...
  - **SCORM 2004 — ✅ ACCEPTED KNOWN-LIMITATION (ship without it).** The manifest `<imsss:deliveryControls completionSetByContent/objectiveSetByContent="true">` fix did **not** work: the live RTE still reads `completion_status=completed, success_status=passed, score.scaled=1, lastError=0`, but SCORM Cloud still reports the registration `UNKNOWN`. There is **no content-side library** for SCORM 2004 (`scorm-again` is LMS-side — confirmed against its README — so it does not fit OELT's content-side runtime). **Decision (Jim, 2026-06-18):** rather than block the toolkit on hand-debugging the 2004 rollup, ship on **SCORM 1.2 + cmi5 + web** and treat 2004 completion rollup as a documented known-limitation. 2004 _packaging_ stays available for authors who knowingly want it; the toolkit never claims 2004 completion is verified. Surfaced honestly across the published surfaces (Task 12): the `scorm2004-rollup` validator warning + `oelt package --target scorm2004` stderr notice (`specs/validator-output.md` §4), `docs/llms.txt` + `docs/quickstart.md` target guidance, and the `tracking-semantics.md` §4 note. Tracked as a **known gap** in `scripts/scorm-cloud/playthroughs.mjs` (`KNOWN_GAPS.scorm2004`): the net reports it as a ⚠ gap and does not fail on it. SCORM 1.2 + cmi5 + web are conformant.
  - **scorm-again is NOT adopted** (wrong side of the API).
- **If reopened:** picking up the 2004 rollup later is a **fresh question** (candidates left as a hint for whoever takes it: `cmi.exit`, a primary objective with `minNormalizedMeasure`, `adlcp:completionThreshold`). When it conforms on a real LMS, remove `scorm2004` from `KNOWN_GAPS` and from the known-limitation notices above.
