...
- **Resolution (in progress, 2026-06-18):**
  - **SCORM 2004 — fixed in the packager (no dependency).** Root cause was the manifest, not the adapter; `scorm-again` would not have helped (it is **LMS-side** — it _provides_ `window.API_1484_11`; confirmed against its README — so it does not fit OELT's content-side runtime). Fix: `scorm2004Manifest` now emits `<imsss:deliveryControls completionSetByContent="true" objectiveSetByContent="true"/>`, telling the LMS the SCO sets its own completion + objective status. Pending verification on the next Cloud run.
  - **cmi5 — patch attempted; `@xapi/cmi5` is the fallback.** Likely root cause: the launch `endpoint` param has no trailing slash, so `…/activities/state` 404s and `start()` throws, blocking render. Fix: normalize the endpoint + make LaunchData hydration non-fatal. `@xapi/cmi5` (content-side AU client, permitted by hard-rule 1) remains the fallback if the hand-written client still can't reach conformance — decide with the next run's captured console error in hand.
  - **Recommendation:** verify both fixes on one full Cloud run; if cmi5 still fails, adopt `@xapi/cmi5` for that target. `scorm-again` is **not** adopted (wrong side of the API).
